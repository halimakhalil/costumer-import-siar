...
function checkLimits(templateSpecifications, app) {
    return groupBy(templateSpecifications, (spec) => spec.types.every((extension) => app.extensionsForType(extension).length < extension.registrationLimit)
...
async function handleTypeParameter(type, app, templateSpecifications) {
    ...
    templateSpecification.types.forEach((spec) => {
        ...
        if (existing.length >= limit) {
            throw new AbortError('Invalid extension type', `You can only generate ${limit} extension(s) of type ${spec.externalIdentifier} per app`);
        }
    });
